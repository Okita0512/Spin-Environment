...
plt.plot(data2[:,0], (data2[:,1] - data2[:,2]), "k-", color = 'red', label = "Prony = 5 + 5, ferr = 5e-7", linewidth = 1.5)    #  * 0.97

data2 = np.loadtxt("1e-5.dat", dtype = float) # _M1_focused
plt.plot(data2[:,0], (data2[:,1] - data2[:,2]), "k--", color = 'green', label = "Prony = 5 + 5, ferr = 1e-5", linewidth = 1.5)    #  * 0.97

data = np.loadtxt(Ref, dtype = float)
plt.plot(data[:,0], data[:,1], "o", color = 'navy', label = "ML-MCTDH")
...
ax.set_title('Spin Bath 4', font = 'Times New Roman', size = 18)
